...
namespace ElasticSearchDemoProject.Suggest {
	...
	public class WeightedProgrammingLanguage {
		public string Language { get; set; }
		public CompletionField Suggest { get; set; }
	}

	public class WeightedSuggest {
		private const string INDEX_NAME = "weightedsuggest";
		private readonly ElasticClient client;

		public WeightedSuggest() {
			// Connection
			...
			client.CreateIndex(INDEX_NAME, ci => ci
				.Mappings(m => m
					.Map<WeightedProgrammingLanguage>(mm => mm
						.AutoMap()
						.Properties(p => p
							.Completion(c => c
								.Name(n => n.Suggest)
			...
			foreach (var language in languages) {
				client.Index(new WeightedProgrammingLanguage() {
					Language = language,
					Suggest = new CompletionField() {Input = new[] {language}, Weight = language.Length }
				});
			}
		...
		public void Demo() {
			...
			while (true)
			{
				...
				var searchResponse = client.Search<WeightedProgrammingLanguage>(s => s
					.Suggest(su => su
